...
module.exports.auth = function authorize() {
  ...
  oauthFile = fs.readFileSync( configFilePath, 'utf8' );
  oauthKeys = yaml.load( oauthFile );

  // console.log( oauthKeys );

  if ( !oauthKeys.consumerKey || !oauthKeys.consumerSecret || !oauthKeys.baseHostname ) {
...
function onAuthComplete( accessToken, accessSecret ) {
  ...
  var content = 'baseHostname: ' + oauthKeys.baseHostname + "\n" +
    'consumerKey: ' + oauthKeys.consumerKey + "\n" +
    'consumerSecret: ' + oauthKeys.consumerSecret + "\n" +
    'accessToken: ' + accessToken + "\n" +
    'accessSecret: ' + accessSecret  + "\n";
  fs.writeFile( configFile, content, 'utf8', function() {

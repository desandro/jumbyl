...
function writeId( id, post ) {
  var file;
  // use quotes so JS doesn't munge the integer weirdly
  var idLine = "id: '" + id + "'\n";
...
module.exports.post = function( filePath ) {
  ...
  var isNew = !params.id;
  // edit if not new
  var method = !isNew ? 'edit' : params.type || 'text';
  client[ method ]( config.base_hostname, params, function( err, data ) {
    if ( err ) {
      console.log( err );
      return;
    }
    // add ID to post front matter
    writeId( data.id, post );
    var action = isNew ? 'created' : 'edited';
    console.log( ( 'Post ' + action + ': ' ).green  + config.base_hostname + '/' + data.id );
  });
...
module.exports.test = function() {
  ...
  client.userInfo( function( err, data ) {
    ...
    console.log( 'Success! Authorized as ' + data.user.name + '.' );
  });
};
